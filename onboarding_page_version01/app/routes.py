...
def get_status():
    ...
    if card_title:
        ...
        # add info dos cards filhos, filtra somente "Testamentary KYC" e "Additional Beneficiary KYC"
        if forms_status:  # check se existem itens em forms_status
            parent_card_id = forms_status[0].get('id')  # select o ID do primeiro form como parent_card_id
            if parent_card_id:
                child_cards = get_child_card_details(parent_card_id)
                for child in child_cards:
                    # filtra os cards que são "Testamentary KYC" ou "Additional Beneficiary KYC"
                    if 'Testamentary KYC' in child['form'] or 'Additional Beneficiary KYC' in child['form']:
                        all_statuses.append({
                            'form': child['form'],
                            'status': child['status'],
                            'url': f"https://app.pipefy.com/open-cards/{child['id']}" if child.get('id') else None  # Adiciona URL do card filho
                        })
