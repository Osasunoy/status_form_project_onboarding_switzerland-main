...
function fetchBankers() {
    fetch('/get_bankers')
        ...
        .then(data => {
            ...
            bankerSelect.innerHTML = '<option value="">Select Banker</option>'; // reset select

            // set para delete duplicatas
            const uniqueBankers = new Set();

            data.forEach(banker => {
                if (!uniqueBankers.has(banker.name)) {
                    uniqueBankers.add(banker.name);
                    const option = document.createElement('option');
                    option.value = banker.id;
                    option.textContent = banker.name;
                    bankerSelect.appendChild(option);
                }
            });
        })
        .catch(error => console.error('Error fetching bankers:', error));
}
